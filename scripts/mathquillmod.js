/*
 * This file is part of SmartCAS (contact@calcenstock.fr) developed for <http://www.calcenstock.fr>.
 * it's a modified version of mathquill.js by :
 *
...
var MathCommand = P(MathElement, function(_, _super) {
  ...
  */
  ...
  _.latex = function() {
    ...
    _.scas = function() {
    return this.foldChildren(this.ctrlSeq, function(scas, child) {
      return scas + '{' + (child.scas() || ' ') + '}';
    });
  };
  _.textTemplate = [''];
  _.text = function() {
...
var Symbol = P(MathCommand, function(_, _super) {
  ...
  _.scas = function(){ return this.textTemplate; };
...
var MathBlock = P(MathElement, function(_) {
  ...
  _.scas = function() { return this.join('scas'); };
  _.text = function() {
...
var RootMathBlock = P(MathBlock, function(_, _super) {
  ...
  _.onKey = function(key, e) {
    ...
    // Prevent newlines from showing up    //remplacer pour evaluer l'expression
    //case 'Enter': evalue(1);
    break;
...
var SupSub = P(MathCommand, function(_, _super) {
  ...
  _.latex = function() {
   ...
   _.scas = function() {
    var scas = this.firstChild.scas();
    if (scas.length === 1)
      return this.ctrlSeq + scas;
    else
      return this.ctrlSeq + '(' + (scas || ' ') + ')';
  };
  _.redraw = function() {
  ...
  _.respace = function() {
	...
	if (this.prev.ctrlSeq === '\\matrix'){
	    var height = this.prev.jQ.height()/2;
		if(this.ctrlSeq === '_') height = -1*height;
		this.jQ.css('vertical-align',height);
	}
	
    if (
      this.prev.ctrlSeq === '\\lim ' || (
        this.prev instanceof SupSub && this.prev.ctrlSeq != this.ctrlSeq
        && this.prev.prev && this.prev.prev.ctrlSeq === '\\lim '
      )
    ) {
      if (!this.limit) {
        this.limit = true;
        this.jQ.addClass('limitlim');
      }
    }
    else {
      if (this.limit) {
        this.limit = false;
        this.jQ.removeClass('limitlim');
      }
    }
...
LatexCmds.fraction = P(MathCommand, function(_, _super) {
  ...
  _.htmlTemplate =
    ...
    + '</span>'
  ;
  _.textTemplate = ['(', 'default.htm', ')'];
  _.finalizeTree = function() {
    this.up = this.lastChild.up = this.firstChild;
    this.down = this.firstChild.down = this.lastChild;
  };
   _.scas = function() {
    var denom = calcthis(this.lastChild.scas());
	if(denom == 0)
	   return msg = 'Erreur : division par 0';
	else
      return '('+this.firstChild.scas()+')/('+this.lastChild.scas()+')';
  };
});

var LiveFraction =
LatexCmds.over =
CharCmds['default.htm'] = P(Fraction, function(_, _super) {
  _.createBefore = function(cursor) {
...
LatexCmds['√'] = P(MathCommand, function(_, _super) {
  ...
  _.textTemplate = ['sqrt(', ')'];
  _.scas = function() {
    return 'sqrt('+this.firstChild.scas()+')';
  };
  _.parser = function() {
...
var NthRoot =
LatexCmds.nroot = P(SquareRoot, function(_, _super) {
  _.ctrlSeq = '\\nroot';
  _.htmlTemplate =
  ...
  _.textTemplate = ['sqrt[', '](', ')'];
   _.scas = function() {
    return 'root('+this.firstChild.scas()+','+this.lastChild.scas()+')';
  };
});
...
var Bracket = P(MathCommand, function(_, _super) {
  ...
  _.latex = function() {
   ...
   _.scas = function() {
    return this.startcas + this.firstChild.scas() + this.endcas;
  };
  _.redraw = function() {
...
LatexCmds.varphi = //W3C or Unicode?
  bind(Variable,'\\varphi ','&#981;');

LatexCmds.phiv = //Elsevier and 9573-13
LatexCmds.phi = //AMS and LaTeX
  bind(Variable,'\\phi ','&phi;');
...
var BigBigSymbol = P(Symbol, function(_, _super) {
  _.init = function(ch, html) {
    _super.init.call(this, ch, '<big><big>'+html+'</big></big>');
  };
});
...
//LatexCmds.lim = bind(BigSymbol,'\\lim ','lim');
LatexCmds.sumgros = bind(BigBigSymbol,'\\sum ','&sum;');
...
//delimiters
LatexCmds.lfloor = bind(VanillaSymbol, '\\lfloor ','&lfloor;' );
LatexCmds.rfloor = bind(VanillaSymbol, '\\rfloor ','&rfloor;' );
...
var Cursor = P(function(_) {
  ...
  _.moveLeftWithin = function(block) {
    if (this.prev) {
      if (this.prev.lastChild) {
	    if(this.prev.lastChild.jQ.hasClass('cache')){ this.appendTo(this.prev.lastChild);this.moveLeftWithin(this.root);}
	  else this.appendTo(this.prev.lastChild);
	  }
      else this.hopLeft();
    }
    else {
      ...
      if (this.parent.prev ) {
	  if(this.parent.prev.jQ.hasClass('cache')){ this.appendTo(this.parent.prev);this.moveLeftWithin(this.root);}
	  else this.appendTo(this.parent.prev);
  ...
  _.moveRightWithin = function(block) {
    ...
    if (this.next) {
	  ...
	  if(this.next.firstChild.jQ.hasClass('cache')){ this.prependTo(this.next.firstChild);this.moveRightWithin(this.root);}
	  else this.prependTo(this.next.firstChild);
	  } 
      else this.hopRight();
    }
    else {
      ...
      if (this.parent.next) {
	  if(this.parent.next.jQ.hasClass('cache')){ this.prependTo(this.parent.next);this.moveRightWithin(this.root);}
	  else this.prependTo(this.parent.next);
	  }
   ...
   * - else check the parent's 'up'default.htm'down' property - if it's a function,
  ...
  _.writeLatex = function(latex) {
    ...
    if(latex.indexOf('matrix')>-1){
	  var termes = latex.split('matrix');
	  latex = termes[0];
	  for(i=2;i<termes.length;i=i+2){
	     latex += '\\fakemat{}'+termes[i];
	  }
	  this.writeLatexMat(latex,termes);
	}else{
    var self = this;
	...
	/*var prec = self.jQ.parents(".expreval");
	var n=1;
    if(prec.find('.fakemat').length > 0){ prec.find('.fakemat').each(function(){
	   var blockId = $(this).attr(mqBlockId),
          currentBlock = MathElement[blockId];
		  //cursor = block && block.cursor;
		  self.appendTo(currentBlock);self.write('a');
	  });
	}; */ 
	}
    return this;
    ...
    _.writeLatexMat = function(x,matr) {
    ...
    var block = latexMathParser.skip(eof).or(all.result(false)).parse(x);
	...
	var n=1;
    $(input).find('.fakemat').each(function(){
	    var blockId = $(this).attr(mqBlockId),
        currentBlock = MathElement[blockId];
        self.prependTo(currentBlock).moveLeft();affmat(matr[n]);
        self.moveRight().backspace();
		n = n+2;
	  });
  ...
  _.testcommand = function(str) {
     var command = str;
	 var nbs = 0;
	    if((this.prev instanceof Variable)){ 
		   var precedent = this.prev;
		   do{
		      command = precedent.latex() + command;
			  precedent = precedent.prev;
		   }while(precedent && (precedent instanceof Variable));
        }
	    if((this.next instanceof Variable)){ 
		   var suivant = this.next;
		   do{
		      nbs = nbs+1;
		      command = command + suivant.latex();
			  suivant = suivant.next;
		   }while(suivant && (suivant instanceof Variable));
        }
		if(command in KeyCmds){  		//this.insertCmd(command);
		    
		    for(var k=0;k<nbs;k++) this.moveRight();
			for(var m=1;m<command.length+1*(str=='');m++) this.backspace();    //+1 si origine = backspace
			if(command=='system') boutsys();
			if(command=='mat') boutmat();
			return this.insertCmd(KeyCmds[command]);
		}else return Variable(str);
  };
  _.insertCh = function(ch) {
    var cmd;
    if (ch.match(/^[a-zA-Z]$/))    //exclude f because want florin : annulé dans smartcas
      {
	     cmd = this.testcommand(ch);                    //Variable(ch);
	  }
  ...
  _.backspace = function() {
    ...
    if (this.deleteSelection());  // pass
      ...
      if (this.prev.isEmpty()){
	    if(this.prev.latex().indexOf('system')>-1) $('#newsyslin').remove();
		//if(this.prev.latex().indexOf('matrix')>-1) $('#boutmat').remove();
        this.prev = this.prev.remove().prev;
		if($(input).mathquill('latex').indexOf('matrix')<0) $('#boutmat').remove();
		if($(input).mathquill('latex').indexOf('idn')<0) $('#boutidn').remove();
	}else
        this.selectLeft();
    }
  ...
  _.deleteForward = function() {
    ...
    this.parent.bubble('redraw');
    this.testcommand('');
	if($(input).mathquill('latex').indexOf('system')<0) $('#newsyslin').remove();
    if($(input).mathquill('latex').indexOf('matrix')<0) $('#boutmat').remove();
	if($(input).mathquill('latex').indexOf('idn')<0) $('#boutidn').remove();
    return this;
  };
  ...
  _.deleteSelection = function() {
    ...
    this.prev = this.selection.first.prev;
    this.next = this.selection.last.next;
	if(this.selection.latex().indexOf('system')>-1) $('#newsyslin').remove();
	//if(this.selection.latex().indexOf('matrix')>-1) $('#boutmat').remove();
...
$.fn.mathquill = function(cmd, latex) {
    ...
    if (arguments.length > 1) {
      return this.each(function() {
        var blockId = $(this).attr(mqBlockId),
          block = blockId && MathElement[blockId];
        if (block)
          block.renderLatex(scas);
      });
    }

    var blockId = $(this).attr(mqBlockId),
      block = blockId && MathElement[blockId];
    return block && block.scas();
  case 'text':
    var blockId = $(this).attr(mqBlockId),
      block = blockId && MathElement[blockId];
  ...
  case 'onKey':                                             //Ajouts pour SmartCAS
        if (arguments.length > 1) 
            return this.each(function () {
                var blockId = $(this).attr(mqBlockId),
                    block = blockId && MathElement[blockId],
           ...
           switch(latex){
			 case 'Left':
                block.cursor.moveLeft();
                break;
            case 'Right':
                block.cursor.moveRight();
                break;
			case 'hopRight':
			    block.cursor.hopRight();
				break;
            case 'Up':
                block.cursor.moveUp();
                break;
            case 'Down':
                block.cursor.moveDown();
                break;
            case 'Backspace' :
...
//Ajouts pour SmartCAS
LatexCmds.diff = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\diff';
  _.htmlTemplate =
      '<span class="fraction non-leaf">'
    +   '<span class="numerator" ><var>d</var></span>'
    +   '<span class="denominator"><var>d</var><span data-aide="diff">&0</span></span>'
    +   '<span style="display:inline-block;width:0">&nbsp;</span>'
    + '</span>'
	+   '<span data-aide="diff">&1</span>'
  ;
   _.scas = function() {
    return 'diff('+this.lastChild.scas()+','+this.firstChild.scas()+')';
  };
});

LatexCmds.diffn = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\diffn';
  _.htmlTemplate =
      '<span class="fraction non-leaf">'
    +   '<span class="numerator"><var>d</var><sup data-aide="diffn">&0</sup></span>'
    +   '<span class="denominator"><var>d</var><span data-aide="diffn">&1</span><sup data-aide="diffn">&2</sup></span>'
    +   '<span style="display:inline-block;width:0">&nbsp;</span>'
    + '</span>'
	+   '<span data-aide="diffn">&3</span>'
  ;
   _.scas = function() {
    if(this.firstChild.scas()!=this.lastChild.prev.scas()) {return '"erreur : ordres différents"';}
    else return 'diff('+this.lastChild.scas()+','+this.firstChild.next.scas()+','+this.firstChild.scas()+')';
  };
});

LatexCmds.prim = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\prim';
  _.htmlTemplate =
      '<span class="non-leaf">'
    +   '<big>&int;</big>'
    +   '<span data-aide="prim">&0</span>'
    +   '<var>d</var>'
    +   '<span data-aide="prim">&1</span>'
    + '</span>'
  ;
   _.scas = function() {
    return 'int('+this.firstChild.scas()+','+this.lastChild.scas()+')';
  };
});

LatexCmds.integ = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\integ';
  _.htmlTemplate =
      '<big>&int;</big>' 
	+   '<span class="int-bornes non-leaf">'
    +   '<span data-aide="integ">&1</span>'
    +   '<span class="int-bornes-inf" data-aide="integ">&0</span>'
	+   '</span>'  
    +   '<span data-aide="integ">&2</span>'
    +   '<var>d</var>'
    +   '<span data-aide="integ">&3</span>'
  ;
  _.scas = function() {
    return 'int('+this.lastChild.prev.scas()+','+this.lastChild.scas()+','+this.firstChild.scas()+','+this.firstChild.next.scas()+')';
  };
});

LatexCmds.somme = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\somme';
  _.htmlTemplate =	
 '<span class="large non-leaf">'
  +'<span class="small" data-aide="somme">&2</span>'	
        +   '<big>&sum;</big>'
    +   '<span class="small non-leaf" data-aide="somme">&0</span>'
    +   '<span class="small non-leaf">=</span>' 
	+   '<span class="small non-leaf" data-aide="somme">&1</span>'
	+   '</span>'
    +   '<span class="non-leaf" data-aide="somme">&3</span>'
  ;
  _.finalizeTree = function() {
    this.up = this.firstChild.up = this.firstChild.next.up = this.lastChild.up = this.lastChild.prev;
	this.lastChild.down = this.lastChild.prev.down = this.firstChild.next;
    this.down = this.firstChild;
  };
  _.scas = function() {
    return 'sum('+this.lastChild.scas()+','+this.firstChild.scas()+','+this.firstChild.next.scas()+','+this.lastChild.prev.scas()+')';
  };
});

LatexCmds.produit = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\produit';
  _.htmlTemplate =	
 '<span class="large non-leaf">'
  +'<span class="small" data-aide="produit">&2</span>'	
        +   '<big>&prod;</big>'
    +   '<span class="small non-leaf" data-aide="produit">&0</span>'
    +   '<span class="small non-leaf">=</span>' 
	+   '<span class="small non-leaf" data-aide="produit">&1</span>'
	+   '</span>'
    +   '<span class="non-leaf" data-aide="produit">&3</span>'
  ;
  _.finalizeTree = function() {
    this.up = this.firstChild.up = this.firstChild.next.up = this.lastChild.up = this.lastChild.prev;
	this.lastChild.down = this.lastChild.prev.down = this.firstChild.next;
  ...
  _.scas = function() {
    return 'product('+this.lastChild.scas()+','+this.firstChild.scas()+','+this.firstChild.next.scas()+','+this.lastChild.prev.scas()+')';
  };
});

LatexCmds.subst  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\subst';
  _.htmlTemplate =
    '<span class="non-leaf" data-aide="subst">&0</span>'
    +      '<big>|</big>'
	+  '<sub class="non-leaf" style="" data-aide="subst">&1</sub>'
    +    '<sub class="non-leaf" style="">=</sub>'
    +'<sub class="non-leaf" style="" data-aide="subst">&2</sub>'
   ; 
   _.scas = function() {
     return 'subst('+this.firstChild.scas()+','+this.firstChild.next.scas()+'='+this.lastChild.scas()+')';
  };
});

LatexCmds.limperso = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\limperso';
  _.htmlTemplate =
   '<span class="limperso non-leaf">'
     +   '<span class="limblock">lim</span>'
	+      '<span class="small" data-aide="lim">&0</span>'
    +      '<span class="small">&rarr;</span>'
    +      '<span class="small" data-aide="lim">&1</span>'
	+'</span>'
    +      '<span class="non-leaf" data-aide="lim">&2</span>'
 ;
   _.scas = function() {
     return 'limit('+this.lastChild.scas()+','+this.firstChild.scas()+','+this.firstChild.next.scas()+')';
  };
});

LatexCmds.limsupinf = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\limsupinf';
  _.htmlTemplate =
   '<span class="limperso non-leaf">'
     +   '<span class="limblock">lim&nbsp;&nbsp;</span>'
	+      '<span class="small" data-aide="limsi">&0</span>'
    +      '<span class="small">&rarr;</span>'
    +      '<span class="small" data-aide="limsi">&1</span>'
	+      '<sup class="small" data-aide="limsi">&2</sup>'
	+'</span>'
    +      '<span class="non-leaf" data-aide="limsi">&3</span>'
 ;
   _.scas = function() {
     return 'limit('+this.lastChild.scas()+','+this.firstChild.scas()+','+this.firstChild.next.scas()+','+this.lastChild.prev.scas()+'1)';
  };
});

LatexCmds.solve = LatexCmds.equation  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\equation';
  _.htmlTemplate =
      '<span class="non-leaf">'
    +      '<span class="non-leaf" data-aide="eq">&0</span>'
    +      '<big>|</big>'
    +  '</span>' 
	+  '<sub class="non-leaf" style=""><var>var=</var></sub>'
    +    '<sub class="non-leaf" style="" data-aide="eq">&1</sub>'
  ;
   _.scas = function() {
     return 'solve('+this.firstChild.scas()+','+this.lastChild.scas()+')';
  };
});

LatexCmds.system = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\system';
    _.htmlTemplate = 
	'<span class="paren scaled">{</span>'
    + '<span class="non-leaf">'
    +   '<span class="system non-leaf">'
    +     '<span data-aide="system">&0</span>'
    +   '</span>'
    + '</span>' 
    +  '<span  class="non-leaf" style="margin-left:10px;vertical-align:bottom">'
    +      '<big>|</big>'	
    +  '<span class="non-leaf" style=""><var>vars=</var></span>'
    +    '<span class="non-leaf" style="" data-aide="system">&1</span>'
    + '</span>' 

	
 /*_.latex = function() {var varsys = this.lastChild.latex(); 
    return '\\system{'+this.foldChildren([], function(latex, child) {
      if(child.latex() != varsys) latex.push(child.latex());
	  return latex;
    }).join(',')+'}' ;
  };*/

   _.scas = function() {var varsys = this.lastChild.scas(); 
    return 'solve(['+this.foldChildren([], function(scas, child) {
      if(child.scas() != varsys && child.scas() != '') scas.push(child.scas());
	  return scas;
    }).join(',')+'],['+varsys+'])' ;
  };
  ...
  _.onKey = function(key, e) {
    ...
    if (currentBlock.parent === this ) {
      ...
      if (key === 'Insert' && currentBlock!=this.lastChild) { 
        var newBlock = MathBlock();
       newBlock.parent = this;
        newBlock.jQ = $('<span></span>')
          .attr(mqBlockId, newBlock.id)
          .insertAfter(currentBlock.jQ)
		  .data('aide','system');       //pour l'affichage dynamique de l'aide
        if (currentBlock.next)
          currentBlock.next.prev = newBlock;
        else
          this.lastChild = newBlock;
		...
		action('1');backsp();
		//this.parent.jQ.mathquill('redraw');
        e.preventDefault();
        ...
        if (currentBlock.isEmpty()) {
		  if(currentBlock!=this.lastChild){
          if (currentBlock.prev) {
            this.cursor.appendTo(currentBlock.prev)
            currentBlock.prev.next = currentBlock.next;
          }
          ...
          if (this.isEmpty())
            {this.cursor.deleteForward();$('#newsyslin').remove();}
          else
            this.bubble('redraw');
      ...
      }else if(key == 'Down' && currentBlock!=this.lastChild) this.cursor.appendTo(currentBlock.next);
	       else if(key == 'Up' && currentBlock!=this.firstChild) this.cursor.appendTo(currentBlock.prev);
    }
  };
});

LatexCmds.sysexemple = P(MathCommand, function(_, _super) {     //pour l'exemple de l'aide
    _.htmlTemplate = 
	'<span class="paren scaled">{</span>'
    + '<span class="non-leaf">'
    +   '<span class="system non-leaf">'
    +   '<span data-aide="system">&0</span>'
    +   '<span data-aide="system">&1</span>'
    +   '<span data-aide="system">&2</span>'
    +   '</span>'
    + '</span>' 
    +  '<span  class="non-leaf" style="margin-left:10px;vertical-align:bottom">'
    +      '<big>|</big>'	
    +  '<span class="non-leaf" style=""><var>vars=</var></span>'
    +    '<span class="non-leaf" style="" data-aide="system">&3</span>'
    + '</span>' 
...
LatexCmds.eqdiff  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\eqdiff';
  _.htmlTemplate =
      '<span class="non-leaf">'
    +      '<span class="non-leaf" data-aide="eqdiff">&0</span>'
    +      '<big>|</big>'
    +  '</span>' 
	+  '<sub class="non-leaf" style=""><var>fct=</var></sub>'
    +    '<sub class="non-leaf" style="" data-aide="eqdiff">&1</sub>'
	+  '<sub class="non-leaf" style=""><var>&nbsp;,var=</var></sub>'
    +    '<sub class="non-leaf" style="" data-aide="eqdiff">&2</sub>'	
  ;
   _.scas = function() {
     fct=this.lastChild.prev.scas();vari=this.lastChild.scas();
     return 'desolve(['+this.firstChild.scas()+'],'+this.lastChild.scas()+','+this.lastChild.prev.scas()+')';
  };
});

LatexCmds.acosinus  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\acosinus';
  _.htmlTemplate =
         '<var>cos</var>'
	+      '<sup class="non-leaf" style="">-1</sup>'
	+ '<span class="non-leaf">'
  ...
  ;
    _.scas = function() {
     return 'acos('+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.asinus  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\asinus';
  _.htmlTemplate =
         '<var>sin</var>'
	+      '<sup class="non-leaf" style="">-1</sup>'
  ...
  ;
    _.scas = function() {
     return 'asin('+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.atangente  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\atangente';
  _.htmlTemplate =
         '<var>tan</var>'
	+      '<sup class="non-leaf" style="">-1</sup>'
  ...
  ;
   _.scas = function() {
     return 'atan('+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.exp = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\exp';
  _.htmlTemplate =	
     '<var>e</var>'
    +'<sup class="non-leaf">&0</sup>'	
  ;

  _.scas = function() {
    return 'e^('+this.firstChild.scas()+')';
  };
});
...
LatexCmds.log  = P(MathCommand, function(_, _super) {
  ...
  ;
   _.scas = function() {
     return 'log10('+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.logb  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\logb';
  _.htmlTemplate =
         '<var>log</var>'
    + '<sub class="non-leaf">&0</sub>'
	+ '<span class="non-leaf">'
    +      '<span class="scaled paren">(</span>'
  ...
  ;
   _.scas = function() {
     return 'logb('+this.lastChild.scas()+','+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ.eq(2);
...
LatexCmds.Re  = LatexCmds.re  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\re';
  _.htmlTemplate =
    '<var>Re</var>'
	+ '<span class="non-leaf">'
    +      '<span class="scaled paren">(</span>'
    +      '<span class="non-leaf" data-aide="re">&0</span>'
    +      '<span class="scaled paren">)</span>'
    +   '</span>'
  ;
  _.scas = function() {
     return 're('+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.Im = LatexCmds.im  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\im';
  _.htmlTemplate =
    '<var>Im</var>'
	+ '<span class="non-leaf">'
    +      '<span class="scaled paren">(</span>'
    +      '<span class="non-leaf" data-aide="im">&0</span>'
    +      '<span class="scaled paren">)</span>'
    +   '</span>'
  ;
  _.scas = function() {
     return 'im('+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.arg  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\arg';
  _.htmlTemplate =
    '<span class="non leaf">&ang;</span>'
	+ '<span class="non-leaf">'
    +      '<span class="scaled paren">(</span>'
    +      '<span class="non-leaf" data-aide="arg">&0</span>'
    +      '<span class="scaled paren">)</span>'
    +   '</span>'
  ;
  _.scas = function() {
     return 'arg('+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.conj  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\conj';
  _.htmlTemplate =
    '<span class="non leaf overline" data-aide="conj">&0</span>'
  ;
  _.scas = function() {
     return 'conj('+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.abs  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\abs';
  _.htmlTemplate =

	'<span class="non-leaf">'
    +      '<span class="scaled paren">&#8739;</span>'
    +      '<span class="non-leaf" data-aide="abs">&0</span>'
    +      '<span class="scaled paren">&#8739;</span>'
    +   '</span>'
  ;
  _.scas = function() {
     return 'abs('+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.binomial = P(MathCommand, function(_, _super) {
  ...
  _.htmlTemplate =
    '<span class="non-leaf">'
    + '<span class="paren scaled parenbinom">(</span>'

    +   '<span class="array non-leaf">'
    +     '<span data-aide="binom">&0</span>'
    +     '<span data-aide="binom">&1</span>'
    +   '</span>'
  ...
  _.scas = function() {
    return 'binomial('+this.firstChild.scas()+','+this.lastChild.scas()+')';
  };
  _.redraw = function() {
    var blockjQ = this.jQ.find('.non-leaf').eq(0);

    var height = blockjQ.outerHeight()/+blockjQ.css('fontSize').slice(0,-2);

    var parens = this.jQ.find('.parenbinom');
    scale(parens, min(1 + .2*(height - 1), 1.2), 1.05*height);
  };
});
...
LatexCmds.matrix = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\matrix';
    _.htmlTemplate = 
	'<span class="non-leaf">'
    +'<span class="paren scaled parenmat">(</span>'
    + '<span class="non-leaf mqtab">'
    +  '<span class="non-leaf mqtabrow">'
    +     '<span class="non-leaf mqtabcell" data-aide="matrix">&0</span>'
    +   '</span>'
	+ '</span>'
    + '<span class="paren scaled parenmat">)</span>'
	+ '</span>'
...
_.latex = function() {
      var nblin = this.jQ.find('.mqtabrow').length;
	  var nbcol = this.jQ.find('.mqtabrow').eq(0).find('.mqtabcell').length;
      var allspan = this.foldChildren([], function(latex, child) {
             latex.push(child.latex());
	      return latex;
         }).join('&');
	  var tabspan = allspan.split('&');
	  var textout = '';
      for(k=0;k<nblin;k++){
	    //exprout += '[';
	    for(l=nbcol*k;l<nbcol*(k+1);l++){
		   textout += tabspan[l];
		   if(l != nbcol*(k+1)-1) textout += '&';
	    }
	  //exprout += ']';
	  if(k != nblin-1) textout += '\\\\';
	  }
	  //exprout += ']';
	  return 'matrix'+textout+'matrix';
  };
  
   _.scas = function() {
      var nblin = this.jQ.find('.mqtabrow').length;
	  var nbcol = this.jQ.find('.mqtabrow').eq(0).find('.mqtabcell').length;
      var allspan = this.foldChildren([], function(scas, child) {
          if(child.scas() != '') scas.push(child.scas());
	      return scas;
         }).join('&');
	  var tabspan = allspan.split('&');
	  var exprout = '[';
      for(k=0;k<nblin;k++){
	    ...
	    for(l=nbcol*k;l<nbcol*(k+1);l++){
		   exprout += tabspan[l];
		   if(l != nbcol*(k+1)-1) exprout += ',';
	    }
	  exprout += ']';
	  if(k != nblin-1) exprout += ',';
	  }
	  exprout += ']';
	  return exprout;
  };
 ...
 /* _.text = function() {
  ...
  _.onKey = function(key, e) {
	...
	if (currentBlock.parent === this ) {
      ...
      if (e.keyCode === 35) { 
	  var self = this;
	  var pos = currentBlock.jQ.index()+1;
	  var n = 0;
	  var first;
	  $('.mqtabcell:nth-child('+pos+')',self.jQ).each(function(){
	      var blockId = $(this).attr(mqBlockId),
          currentBlock = MathElement[blockId];
        ...
        newBlock.jQ = $('<span></span>')
          .attr(mqBlockId, newBlock.id)
          .insertAfter(currentBlock.jQ)
		  .data('aide','matrix')                //pour l'affichage dynamique de l'aide
		  .addClass('mqtabcell');       
        if (currentBlock.next)
          currentBlock.next.prev = newBlock;
        else
          self.lastChild = newBlock;
        if(n == 0) first = newBlock;
        newBlock.next = currentBlock.next;
        currentBlock.next = newBlock;
        newBlock.prev = currentBlock;
		self.cursor.appendTo(newBlock);
		action('1');self.cursor.backspace(); 
		n++;
	  });
        self.cursor.appendTo(first);
        e.preventDefault();
		
        return false;
      }else if (key === 'Insert'){
	     var tr = currentBlock.jQ.parent();
		 var newtr = $('<span></span>').insertAfter(tr).addClass('non-leaf mqtabrow');
		 var nbcol = tr.find('.mqtabcell').length;
         var blockId = $('.mqtabcell:last',tr).attr(mqBlockId),
          currentBlock = MathElement[blockId];
		  var n = 0;
	      var first;
		  for(var k=0;k<nbcol;k++){
            ...
            newBlock.prev = currentBlock;
		    this.cursor.appendTo(newBlock);
		    action('1');this.cursor.backspace();
			currentBlock = newBlock;
			n++
		  }
		  this.cursor.appendTo(first);
	       e.preventDefault();
		  return false;
        }else  if (e.which === 8) { //backspace
          if (currentBlock.isEmpty()) {
          ...
          return false;
		      ...
		      for(var k=0;k<nbcol;k++){
		      if(currentBlock != this.lastChild) 
		         {this.cursor.appendTo(currentBlock.next);currentBlock = currentBlock.next;}
		      else
		         {this.cursor.appendTo(this.firstChild);currentBlock = this.firstChild;}
		      }		
	        }else if(key == 'Up') {
	          var nbcol = this.jQ.find('.mqtabrow').eq(0).find('.mqtabcell').length;
		      for(var k=0;k<nbcol;k++){
		      if(currentBlock != this.firstChild) 
		         {this.cursor.appendTo(currentBlock.prev);currentBlock = currentBlock.prev;}
		      else
		         {this.cursor.appendTo(this.lastChild);currentBlock = this.lastChild;}
		      }
	  }
    }
  };
   _.redraw = function() {
    ...
    var parens = this.jQ.find('.parenmat');
	
    scale(parens, min(1 + .2*(height - 1), 1.2), 1.05*height);
  };
});

LatexCmds.fakemat = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\fakemat';
    _.htmlTemplate = 
       '<span class="fakemat">&0</span>'
  ;
  _.latex = function() {
      return this.firstChild.latex();
  };
...
LatexCmds.exempmat = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\exempmat';
    _.htmlTemplate = 
	'<span class="non-leaf">'
    +'<span class="paren scaled">(</span>'
    + '<span class="non-leaf mqtab">'
    +  '<span class="non-leaf mqtabrow">'
    +     '<span class="non-leaf mqtabcell">&0</span>'
	+     '<span class="non-leaf mqtabcell">&1</span>'
    +   '</span>'
	+  '<span class="non-leaf mqtabrow">'
    +     '<span class="non-leaf mqtabcell">&2</span>'
	+     '<span class="non-leaf mqtabcell">&3</span>'
    +   '</span>'
	+  '<span class="non-leaf mqtabrow">'
    +     '<span class="non-leaf mqtabcell">&4</span>'
	+     '<span class="non-leaf mqtabcell">&5</span>'
    +   '</span>'
	+ '</span>'
    + '<span class="paren scaled">)</span>'
	+ '</span>'
  ;
   _.redraw = function() {
    ...
    var parens = this.jQ.find('.paren');
	
    scale(parens, min(1 + .2*(height - 1), 1.2), 1.05*height);
  };
});

LatexCmds.exempmatcar = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\exempmat';
    _.htmlTemplate = 
	'<span class="non-leaf">'
    +'<span class="paren scaled">(</span>'
...
LatexCmds.idn = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\idn';
    _.htmlTemplate = 
	  '<span class="non-leaf">'
	  + '<var>I</var>'
      + '<sub data-aide="idn">&0</sub>'
	  + '</span>'
  ;
  _.scas = function() {
      var taille = calcthis(this.firstChild.scas());
	  var matr = '';
  ...
  _.onKey = function(key, e) {
            var blockId = $(input).attr(mqBlockId),
	...
	if (cursor.parent.parent === this ) {
      ...
      if (key === 'Insert') {
			...
			cursor.insertAfter(this).backspace().backspace();
			affmat(matr);
            if($(input).mathquill('latex').indexOf('idn')<0) $('#boutidn').remove();
			boutmat();
...
LatexCmds.idnnn = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\idnnn';
    _.htmlTemplate = 
	'<span class="non-leaf">'
    +'<span class="paren scaled">(</span>'
...
LatexCmds.tran  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\tran';
  _.htmlTemplate =

	'<span class="non-leaf" style="padding-top_3A0.35em">'
    +      '<var style="vertical-align:top;display:inline-block;margin-top:-0.5em">t</var>'
    +      '<span class="non-leaf" data-aide="tran">&0</span>'

    +   '</span>'
  ;
  _.scas = function() {
     return 'tran('+this.firstChild.scas()+')';
  };

});

LatexCmds.det  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\det';
  _.htmlTemplate =
     '<var>det</var>'
	+     '<span class="non-leaf">'
    +      '<span class="scaled paren">(</span>'
    +      '<span class="non-leaf" data-aide="det">&0</span>'
    +      '<span class="scaled paren">)</span>'
    +   '</span>'
  ;
  _.scas = function() {
     return 'det('+this.firstChild.scas()+')';
  };
      _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.floor  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\floor';
  _.htmlTemplate =

	'<span class="non-leaf">'
    +      '<span class="scaled paren">&lfloor;</span>'
    +      '<span class="non-leaf" data-aide="floor">&0</span>'
    +      '<span class="scaled paren">&rfloor;</span>'
    +   '</span>'
  ;
  _.scas = function() {
     return 'floor('+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.pgcd  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\pgcd';
  _.htmlTemplate =
    '<var>pgcd</var>'
	+  '<span class="non-leaf">'
    +      '<span class="scaled paren">(</span>'
    +      '<span class="non-leaf" data-aide="pgcd">&0</span>'
    +      '<span class="scaled paren">)</span>'
    +   '</span>'
  ;
  _.scas = function() {
     return 'gcd('+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.ppcm  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\ppcm';
  _.htmlTemplate =
    '<var>ppcm</var>'
	+  '<span class="non-leaf">'
    +      '<span class="scaled paren">(</span>'
    +      '<span class="non-leaf" data-aide="ppcm">&0</span>'
    +      '<span class="scaled paren">)</span>'
    +   '</span>'
  ;
  _.scas = function() {
     return 'lcm('+this.firstChild.scas()+')';
  };
    _.jQadd = function() {
    _super.jQadd.apply(this, arguments);
    var jQ = this.jQ;
...
LatexCmds.quotient  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\quotient';
  _.htmlTemplate =

     '<span class="non-leaf dividende" data-aide="quotient">&0</span>'
    +  '<span class="scaled paren">&#8739;</span>'
	 +  '<span class="non-leaf">'
    +'<span class="array non-leaf">'
    +   '<span class="non-leaf diviseur" data-aide="quotient">&1</span>'
		+ '<span class="non-leaf"><var>q</var>?</span>'

	+ '</span>'
	+ '</span>'	
  ;
  _.scas = function() {
     return 'iquo('+this.firstChild.scas()+','+this.lastChild.scas()+')';
  };
     _.redraw = function() {
    var blockjQ = this.jQ.eq(2);
...
LatexCmds.reste  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\reste';
  _.htmlTemplate =
    '<span class="non-leaf">'
    +'<span class="array non-leaf">'
    + '<span class="non-leaf" data-aide="reste">&0</span>'
	+ '<span class="non-leaf"><var>r</var>?</span>'
	+ '</span>'
	+ '</span>'
    +  '<span class="scaled paren">&#8739;</span>'
    +   '<span class="non-leaf diviseur" data-aide="reste">&1</span>'	
  ;
  _.scas = function() {
     return 'irem('+this.firstChild.scas()+','+this.lastChild.scas()+')';
  };
     _.redraw = function() {
...
LatexCmds.resquo  = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\resquo';
  _.htmlTemplate =
    '<span class="non-leaf">'
    +'<span class="array non-leaf">'
    + '<span class="non-leaf" data-aide="resquo">&0</span>'
	+ '<span class="non-leaf"><var>r</var>?</span>'
	+ '</span>'
	+ '</span>'
    +  '<span class="scaled paren">&#8739;</span>'
	 +  '<span class="non-leaf">'
    +'<span class="array non-leaf">'
    +   '<span class="non-leaf diviseur" data-aide="resquo">&1</span>'
		+ '<span class="non-leaf"><var>q</var>?</span>'
	+ '</span>'
	+ '</span>'		
  ;
  _.scas = function() {
     return 'iquorem('+this.firstChild.scas()+','+this.lastChild.scas()+')';
  };
     _.redraw = function() {
    var blockjQ = this.jQ.eq(0);
...
LatexCmds.modulo = P(MathCommand, function(_, _super) {
  _.ctrlSeq = '\\modulo';
  _.htmlTemplate =
      '<span class="non-leaf">'
    +   '<span data-aide="modulo">&0</span>'
    +   '<span class="nonSymbola">%</span>'
    +   '<span data-aide="modulo">&1</span>'
    + '</span>'
  ;
   _.scas = function() {
    return this.firstChild.scas()+'%'+this.lastChild.scas();
...
LatexCmds.def = bind(BinaryOperator,':=',':=',':=');
LatexCmds.esp = LatexCmds.emsp = bind(VanillaSymbol,'\\esp',' ',' ');
//fin ajouts
